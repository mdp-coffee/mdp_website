...
function EventContent({ event, align }: { event: TimelineEntry; align: "left" | "right" }) {
  return (
    <div className={`flex flex-col ${align === "right" ? "items-end text-right" : "items-start text-left"}`}>
      ...
      <p className="mt-1 font-sans text-xs leading-relaxed text-brown/55">{event.detail}</p>
    </div>
  );
}
...
export function CompactTimeline() {
  ...
  return (
    ...
    >
      ...
      <h2 className="mb-8 text-center font-condensed text-2xl not-italic text-brown md:text-3xl">
      ...
      <div className="relative mx-auto w-full max-w-[900px]">
        ...
        <motion.div
          aria-hidden="true"
          className="absolute bottom-0 left-1/2 top-0 hidden w-px -translate-x-1/2 origin-top bg-brown/15 md:block"
              ...
              <div className="relative hidden md:grid md:grid-cols-2">
                <span
                  ...
                  className={`absolute left-1/2 top-1/2 z-10 h-2 w-2 -translate-x-1/2 -translate-y-1/2 rounded-full ${
                    event.active ? "bg-rust" : "bg-brown/20"
                  }`}
                />
                {event.side === "left" ? (
                  <span
